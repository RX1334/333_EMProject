...
def lab_summaries():
    ...
    dashboard_content += render_template('lab-summary-widget.html', lab_id='rabinowitz_icahn_201')
...
def fumehood_summary():
    ...
    dashboard_content = render_template('header-widget.html', page_name=fumehood_id, back_arrow_link='/lab_summary?lab_id=' + lab_id)
...
def lab_summary():
    ...
    page_name = urllib.parse.unquote(lab_id)
    print(page_name)

    # render energy and power widgets
    dashboard_content = render_template('header-widget.html', page_name=page_name, lab_id=lab_id)
    dashboard_content += render_template('heading-label.html', text='Statistics')
    ...
    # bar chart widget
    dashboard_content += render_template('heading-label.html', text='Visualizations')
    dashboard_content += render_template('barchart-widget-json.html', name=lab_id,
...
def report():
    ...
    dashboard_content += render_template('barchart-report.html', name=week_name, lab_id='rabinowitz_icahn_201', type_of_graph='Energy Consumption Trend')
...
# args: optional for either lab_id or fumehood_id (if neither provided,
#       just return None)
